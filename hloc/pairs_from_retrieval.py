...
def main(
    ...
    if not os.path.exists(output):
        ...
        logger.info(f"Already retrived. Skipping retrieval.")
    ...
    query_names_array = np.array(query_names)
    db_names_array = np.array(db_names)

    # Creating a mask where queries are compared against themselves or other queries
    query_self_mask = np.isin(db_names_array, query_names_array)  # If elements of db_names in query_names, True
    query_self_mask = np.tile(query_self_mask, (len(query_names_array), 1))  # Extend to same size of query_names
    self = query_self_mask
