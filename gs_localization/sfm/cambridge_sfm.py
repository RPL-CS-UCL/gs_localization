...
from hloc.utils.sfm_utils import create_query_list_with_intrinsics2
...
def image_path_to_rendered_depth_path1(image_name):
    name = image_name.replace("png", f"depth.tiff")
    return name

def image_path_to_rendered_depth_path2(image_name):
    name = image_name.replace("png", f"depth.png")
    return name
...
def correct_sfm_with_depth(sfm_path, depth_folder_path, output_path):
    ...
    for imgid, img in tqdm(images.items()):
        ...
        image_name = img.name
        depth_name1 = image_path_to_rendered_depth_path1(image_name)
        depth_name2 = image_path_to_rendered_depth_path2(image_name)
        depth_path1 = Path(depth_folder_path) / depth_name1
        depth_path2 = Path(depth_folder_path) / depth_name2
        if depth_path1.exists():
            depth = PIL.Image.open(depth_path1)
        else:
            depth = PIL.Image.open(depth_path2)

        depth = depth.resize((1024, 576), PIL.Image.NEAREST)
        depth = np.array(depth).astype("float64")
        depth = depth / 1000.0  # mm to meter
        depth[(depth < 0.001) | (depth > 1000.0)] = np.nan
        ...
        for i, p3did in enumerate(img.point3D_ids[img.point3D_ids != -1]):
            dist = np.linalg.norm(scs[i]-points3D[p3did].xyz)
            norm_cali = np.linalg.norm(scs[i])
            norm = np.linalg.norm(points3D[p3did].xyz)
            if dist/norm_cali-dist/norm < 0.001 and dist < 2:
...
def create_reference_sfm(full_model, ref_model, whitelist=None, ext=".txt"):
    ...
    write_model(cameras, images_ref, points3D_ref, ref_model, ".txt")
...
def run_scene(
    ...
    train_list = input / "train_fewshot.txt"
    test_list = input / "test_fewshot.txt"
    create_reference_sfm(gt_dir, ref_sfm_sift, train_list)

    cameras_sfm_sift = ref_sfm_sift / "cameras.txt"
    modify_camera_model(cameras_sfm_sift)

    model = f"D:/gs-localization/datasets/cambridge/CambridgeLandmarks_Colmap_Retriangulated_1024px/{scene}/empty_all"
    create_query_list_with_intrinsics2(model, query_list, blacklist=[], list_file=test_list, ext=".txt")
    ...
    #pairs_from_exhaustive.main(sfm_pairs, image_list=input/"train_fewshot.txt", ref_list=input/"train_fewshot.txt")
...
# Create a dictionary that maps scene names to their corresponding values
scene_to_value = dict(zip(SCENES, values))
...
if __name__ == "__main__":
    ...
    for scene in args.scenes:
        args.num_retrieve = scene_to_value.get(scene, None)
        ...
        args.num_retrieve = scene_to_value.get(scene, None)
        depth_dir = Path("datasets")/f"Cambridge_additional/{scene}/train"

        if args.overwrite or True:
            run_scene(
                args.dataset / scene,
                args.outputs / scene,
                args.dataset / scene / "images",
        ...
        list_file = args.dataset / scene / "test_fewshot.txt"
        model = f"D:/gs-localization/datasets/cambridge/CambridgeLandmarks_Colmap_Retriangulated_1024px/{scene}/empty_all"
        evaluate(model, all_results[scene], list_file)
