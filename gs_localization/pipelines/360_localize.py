...
percentage_rot_error = 0
percentage_trans_error = 0
count = 0
...
for scene in ['bicycle', 'bonsai', 'counter', 'garden',  'kitchen', 'room', 'stump', "flowers", "treehill"]:
    ...
    Model.load_ply(f"D:/gs-localization/output/360_v2/{scene}/gs_map/iteration_30000/point_cloud.ply")
    ...
    poses_bounds = np.load(f"D:/gs-localization/datasets/360_v2/{scene}/poses_bounds.npy") # (N_images, 17)
    ...
    for i, image in enumerate(tqdm(test_infos, desc="Localization")):
        ...
        trans_errors.append(trans_error/scale_factor)

        if trans_error/scale_factor < 0.05:
            percentage_trans_error += 1
        if rot_error < 5:
            percentage_rot_error += 1
    
    np.save(f"D:/gs-localization/output/360_v2/{scene}/rot_errors.npy", rot_errors)
    np.save(f"D:/gs-localization/output/360_v2/{scene}/trans_errors.npy", trans_errors)
    med_t = np.median(trans_errors)
    med_R = np.median(rot_errors)
    print( f"\nMedian errors for {scene}: {med_t:.3f}unit, {med_R:.3f}deg")
    ...
    for th_t, th_R in zip(threshs_t, threshs_R):
        ...
        print(f"\n\t{th_t:.0f}unit, {th_R:.0f}deg : {ratio*100:.2f}%")
        
    file.close()

percentage_rot_error = percentage_rot_error/count
percentage_trans_error = percentage_trans_error/count

print(f"percentage rot within 5 degree: {percentage_rot_error}")
print(f"percentage trans within 0.05 unit: {percentage_trans_error}")
