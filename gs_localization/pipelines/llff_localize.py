...
class llff_Dataset(MonocularDataset):
    def __init__(self, args, path, config, data_folder, scene):
        super().__init__(args, path, config)
        self.has_depth = False
        self.llff_Parser(data_folder, scene) 
        
    def llff_Parser(self, data_folder, scene):
...
for scene in ['fern', 'flower', 'fortress',  'horns',  'leaves',  'orchids',  'room',  'trex']:
    ...
    dataset = llff_Dataset(model_params, model_params.source_path, config, data_folder, scene)
    bg_color = [0, 0, 0] 
    ...
    poses_bounds = np.load(f"D:/gs-localization/datasets/nerf_llff_data/{scene}/poses_bounds.npy") # (N_images, 17)
    ...
    np.save(f"D:/gs-localization/output/nerf_llff_data/{scene}/rot_errors.npy", rot_errors)
    np.save(f"D:/gs-localization/output/nerf_llff_data/{scene}/trans_errors.npy", trans_errors)
