    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            if line.startswith(current_scene):
                ...
                image_name = f"{seq}_{frame}.png"  # Modify this if your images have a different extension
                fewshot_images.add(image_name)
    return fewshot_images


def generate_fewshot_file(scene, base_path):
    import os
    import shutil
    
    # Input file path
    train_file_path = os.path.join(base_path, 'train_fewshot_all.txt')
    test_file_path = os.path.join(base_path, 'test_Cambridge.txt')
    
    # Output file path
    output_train_file_path = os.path.join(base_path, scene, 'train_fewshot.txt')
    output_test_file_path = os.path.join(base_path, scene, 'test_fewshot.txt')
    
    # Train image output directory
    train_images_dir = os.path.join(base_path, scene, 'train_images')
    ...
    images_dir = os.path.join(base_path, scene, 'images')
    if os.path.exists(images_dir):
        shutil.rmtree(images_dir)
    os.mkdir(images_dir)

    depths_dir = os.path.join(base_path, scene, 'depths')
    if os.path.exists(depths_dir):
        shutil.rmtree(depths_dir)
    os.mkdir(depths_dir)

    train_depths_dir = os.path.join(base_path, scene, 'train_depths')
    ...
    with open(train_file_path, 'r') as infile, open(output_train_file_path, 'w') as train_outfile:
        for line in infile:
            ...
            scene_name, sequence_frame = line.split(' ')
            sequence, frame = sequence_frame.split('_')
            formatted_name = f"{sequence}_{frame}.png"
            ...
            if scene_name == scene:
                ...
                source_path = os.path.join(base_path, scene_name, sequence, frame+".png")
                ...
                depth_name_in_folder = f"{sequence}_{frame}.depth.tiff"
                source_path = os.path.join(base_path.replace("cambridge","Cambridge_additional"), scene, depth_name_in_folder)
                
                target_path = os.path.join(depths_dir, image_name_in_folder.replace(".png",".depth.tiff"))
                target_path2 = os.path.join(train_depths_dir, image_name_in_folder.replace(".png",".depth.tiff"))

                if os.path.exists(source_path):
                    shutil.copy(source_path, target_path)
                    shutil.copy(source_path, target_path2)
                else:
                    print(f"Warning: {source_path} does not exist and cannot be copied.")
    ...
    with open(test_file_path, 'r') as infile, open(output_test_file_path, 'w') as test_outfile:
        for line in infile:
            ...
            if scene_name == scene:
                ...
                image_name_in_folder = formatted_name
                source_path = os.path.join(base_path, scene_name, sequence, frame+".png")
                target_path = os.path.join(images_dir, image_name_in_folder)
    ...
    print(f"Fewshot file generated: {output_train_file_path}")
    print(f"Training images copied to: {train_images_dir}")
    print(f"Test fewshot file generated: {output_test_file_path}")
...
for scene in ['GreatCourt']:
    ...
    generate_fewshot_file(scene, base_path='D:/gs-localization/datasets/cambridge/')
