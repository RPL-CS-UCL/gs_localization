...
def generate_fewshot_file(scene, base_path):
    ...
    with open(train_file_path, 'r') as infile, open(output_train_file_path, 'w') as train_outfile:
        for line in infile:
            ...
            scene_name, sequence, frame = line.split(' ')
            formatted_name = f"{sequence}-{frame}-color.png"
            
            if scene_name == scene:
                train_outfile.write(f"{formatted_name}\n")
                
                # Copy the corresponding images to the train_images_dir directory
                image_name_in_folder = formatted_name.replace('/', '-')
                source_path = os.path.join(base_path, scene_name, sequence, frame+".color.png")
                ...
                depth_name_in_folder = f"{sequence}/{frame}.depth_cali.png"
                source_path = os.path.join(base_path.replace("7scenes","7Scenes_additional"), scene, depth_name_in_folder)
                target_path = os.path.join(depths_dir, image_name_in_folder.replace("color","depth"))
                target_path2 = os.path.join(train_depths_dir, image_name_in_folder.replace("color","depth"))
    ...
    # Process the sequences specified in TestSplit.txt and 
...
#for scene in ["stairs"]:
    ...
    generate_fewshot_file(scene, base_path='D:/gs-localization/datasets/7scenes/')
