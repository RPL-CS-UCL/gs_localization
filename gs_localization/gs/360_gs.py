...
def copy_images_and_setup_sfm(dataset, outputs, scene):
    ...
    images_src_dir = Path(dataset) / scene / 'images_4'
    ...
    for image_file in images_src_dir.glob('*'):
        if image_file.is_file():
            shutil.copy(image_file, images_dst_dir / image_file.name)
    ...
    for sfm_file in sfm_src_dir.glob('*'):
        if sfm_file.is_file():
            shutil.copy(sfm_file, sparse_dst_dir / sfm_file.name)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if use_depth == True:
            ...
            loss += 0.02 * l1_loss(depth*depth_mask, gt_depth*depth_mask)
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {} Pearson {}".format(
                            iteration, config['name'], l1_test, psnr_test, 0.005*pearson_test))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        default=Path(__file__).resolve().parent.parent.parent/"datasets/360_v2",
        help="Path to the dataset, default: %(default)s",
    )
    parser.add_argument(
        "--outputs",
        type=Path,
        default=Path(__file__).resolve().parent.parent.parent/"output/360_v2",
    ...
    for scene in SCENES:
        ...
        if scene in ['bicycle', 'garden', 'stump']:
            print("Optimizing ", args.model_path)

            # Update lp for the current scene
            args.source_path = Path(args.outputs)/scene
            args.model_path = Path(args.outputs)/scene

            # Update op for the current scene
            args.densification_interval = 200
