...
def copy_images_and_setup_sfm(dataset, outputs, scene):
    ...
    for image_file in images_src_dir.glob('*'):
        if image_file.is_file() and not os.path.exists(images_dst_dir / image_file.name):
            os.symlink(image_file, images_dst_dir / image_file.name) 
    for depth_file in depths_src_dir.glob('*'):
        if depth_file.is_file() and not os.path.exists(depths_dst_dir / depth_file.name):
            img = Image.open(depth_file)
            img_resized = img.resize((1024, 576), resample=Image.NEAREST)
            img_resized.save(depths_dst_dir / depth_file.name)
    ...
    for sfm_file in sfm_src_dir.glob('*'):
        if sfm_file.is_file() and not os.path.exists(sparse_dst_dir / sfm_file.name):
            os.symlink(sfm_file, sparse_dst_dir / sfm_file.name)

def delete_directories(outputs, scene):
    # Define the paths for the images and sparse/0 directories
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if torch.sum(midas_depth) > 10:
            ...
            loss += 0.005 * pseudo_depth_loss
        else:
            print("Caution: this camera has no ground truth depth.")

        # l1 depth loss
        if use_depth == True:
            gt_depth = viewpoint_cam.depth.clone().detach().reshape(-1,1)
            depth_mask = (gt_depth>0.01)
            #loss += (-1e-6 * iteration + 0.05) * l1_loss(depth*depth_mask, gt_depth*depth_mask)
            loss += 0.01 * l1_loss(depth*depth_mask, gt_depth*depth_mask)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, viewpoint_stack):
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = [{'name': 'train', 'cameras': viewpoint_stack}]
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {} Depth {} ".format(
                            iteration, config['name'], l1_test, psnr_test, 0.02*depth_test))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        default=Path(__file__).resolve().parent.parent.parent/"datasets/cambridge",
        help="Path to the dataset, default: %(default)s",
    )
    parser.add_argument(
        "--outputs",
        type=Path,
        default=Path(__file__).resolve().parent.parent.parent/"output/cambridge_full",
    ...
    for scene in SCENES:
        ...
        training(lp.extract(args), 
                    op.extract(args), 
                    pp.extract(args), 
                    [7_000, 10_000, 15_000, 20_000, 25_000, 27_500, 30_000], 
                    [30_000], 
        ...
        gs_map_dir = Path(args.outputs) / scene / 'gs_map_larger_depth'
