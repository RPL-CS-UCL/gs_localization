...
def copy_images_and_setup_sfm(dataset, outputs, scene):
    ...
    depths_src_dir = Path(dataset) / scene / 'train_depths'
    ...
    for depth_file in depths_src_dir.glob('*'):
        if depth_file.is_file():
            shutil.copy(depth_file, depths_dst_dir / depth_file.name)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {} Depth {} Pearson {}".format(
                            iteration, config['name'], l1_test, psnr_test, 0.02*depth_test, 0.005*pearson_test))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--outputs",
        type=Path,
        default="D:/gs-localization/output/7scenes",
    ...
    for scene in SCENES:
        ...
        gs_map_dir = Path(args.outputs) / scene / 'gs_map_monodepth'
